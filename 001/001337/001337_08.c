/*
 * 001337_08.c -- Exact memory-intensive enumeration of fixed-endpoint
 *                f.c.c. self-avoiding walks and OEIS A001337.
 *
 *   OEIS A003287 : n-step f.c.c. SAWs from (0,0,0) to (0,1,1)
 *   OEIS A001337 : n-step f.c.c. polygons, = 12 * A003287(n-1) for n >= 3
 *   OEIS A003288 : ... to (0,0,2)      A005543 : ... to (0,2,2)
 *   OEIS A005544 : ... to (1,1,2)      A005545 : ... to (0,1,3)
 *   OEIS A005546 : ... to (0,3,3)      A005547 : ... to (1,2,3)
 *   OEIS A005548 : ... to (2,2,2)
 *
 * METHOD
 * ------
 ...
 *
 * Join points are reduced modulo the stabiliser of the endpoint pair inside
 * the f.c.c. point group (all signed coordinate permutations).  When the
 * halves have equal length the two endpoints may also be exchanged; otherwise
 * their order is retained.  The stabiliser is computed at run time, so every
 * target accepted by walk mode may be used for cross-validation.
 *
 * This file is based on the independently written MIT-licensed
 * 001337_07.c by manman4.  That implementation was derived from the
 * published mathematical description and contains no SAWdoubler source code.
 *
 * SPDX-License-Identifier: MIT
 *
 * Build:   clang -O3 -std=c11 001337_08.c -o 001337_08
 *          gcc-omp 001337_08.c -o 001337_08
 *
 * Optimisations over 001337_07.c, both exact; the degenerate two-step closed
 * case is handled separately (--cutoff invariance and the brute-force
 * cross-check still hold):
 *
 *   1. Join points are reduced modulo the stabiliser of the *unordered* pair
 *      {origin, target}, not just the ordered one.  The extra element is the
 *      point reflection v -> C - v, a lattice automorphism that exchanges the
 *      two endpoints and hence maps one half-walk family onto the other.  It
 *      is only a symmetry when the halves have equal length, so it is enabled
 *      exactly when na == nb.  Roughly halves the join points.
 *
 *   2. Rows store steps-1 sites instead of steps.  A half-walk that touches
 *      the opposite endpoint meets every row of the other bag there, so it can
 *      never belong to a disjoint pair and is rejected during generation.
 *      Afterwards each bag's own start point lies on all of its rows and none
 ...
 * is inside partition_on, and that loop is memory-latency bound, not compute
 * bound.  Adding a sketch-based pre-filter, software prefetch, or an early
 * "past it, give up" exit to the membership scan all measured *slower* -- the
 * last one by 60%, because it stops the compiler vectorising a fixed-width
 * comparison.  Reduce the number of scans, not the cost of one.
 *
 * For walk mode with target (0,0,0), the final return to the origin is
 * permitted and the result is the rooted closed-walk count A001337(L).
 *
...
/*
 * Integer bounds for the supported commands:
 * - polygon indices 1..19 reduce to fixed-endpoint walks of at most 18 steps;
 * - walk mode directly permits 1..18 steps;
 * - both modes split a walk into halves of length at most 9;
 ...
 * - a fixed-endpoint walk count is at most 12*11^17 < 2^63;
 * - an A001337 result is at most 12*11^17 < 2^63 because its final
 *   closing step is forced.
...
static void bag_add(Bag *b, const int *path)
{
    ...
    for (i = 1; i < b->width; i++) {           /* insertion sort, width <= 8 */
        uint16_t v = r[i];
        j = i - 1;
                    ...
                    int last, int depth)
{
    ...
    if (total < 0) die("internal inclusion-exclusion invariant failed");
    return total;
}
...
typedef struct {
    int  stab[96];
    int  flip[96];      /* 1 = compose with the point reflection v -> C - v */
    int  nstab;
    int  target[3];
} Stabiliser;

/* Image of v under element k: g(v), or C - g(v) when the element carries the
   swap.  The point reflection through C/2 is a lattice automorphism (it
   preserves coordinate-sum parity and all distances) and it exchanges the two
   endpoints, so it maps an na-step walk O -> X onto an na-step walk C -> C-X.
   It is therefore a symmetry of the join-point sum only when the two halves
   have equal length; the caller decides. */
static void stab_image(const Stabiliser *st, int k, const int *v, int *out)
{
    int i;
    sym_apply(st->stab[k], v, out);
    if (st->flip[k])
        for (i = 0; i < 3; i++) out[i] = st->target[i] - out[i];
}

static void stabiliser_of(const int *target, int allow_swap, Stabiliser *st)
{
    int g, img[3], i;

    st->nstab = 0;
    for (i = 0; i < 3; i++) st->target[i] = target[i];

    for (g = 0; g < 48; g++) {
        sym_apply(g, target, img);
        if (img[0] == target[0] && img[1] == target[1] && img[2] == target[2]) {
            st->stab[st->nstab] = g; st->flip[st->nstab] = 0; st->nstab++;
        }
    }
    if (allow_swap) {
        int n = st->nstab, j;
        for (j = 0; j < n; j++) {
            st->stab[st->nstab] = st->stab[j];
            st->flip[st->nstab] = 1;
            st->nstab++;
        }
    }
...
static i128 solve_join(const Problem *pr, int join)
{
    ...
    bag_init(&A, pr->nsteps_a - 1);
    bag_init(&B, pr->nsteps_b - 1);

    w.g = g; w.dist = dist; w.seen = seen; w.path = path;

    w.steps = pr->nsteps_a; w.bag = &A; w.banned = pr->target;
    path[0] = pr->origin;
    walk_rec(&w, 0);

    if (A.count > 0) {
        w.steps = pr->nsteps_b; w.bag = &B; w.banned = pr->origin;
        path[0] = pr->target;
        walk_rec(&w, 0);
    }
                        ...
                        int verbose)
{
    ...
    if (nsteps > MAX_WALK_STEPS)
        die("internal walk length out of range");
    if (nsteps == 1) {
        for (i = 0; i < 12; i++)
    ...
    if (nsteps == 2 && target[0] == 0 && target[1] == 0 &&
        target[2] == 0)
        return 0; /* traversing one undirected edge twice is not a polygon */
    for (i = 0; i < 3; i++)
        if (target[i] < -nsteps || target[i] > nsteps) return 0;
...
static int selftest(size_t cutoff)
{
    ...
    printf("[6] coincident endpoint, walk n 0 0 0 must equal A001337(n)\n");
    {
        static const int O[3] = { 0, 0, 0 };
        for (n = 1; n <= 10; n++) {
            i128 got = count_walks(n, O, cutoff, 0);
            int ok = (got == (i128)A001337[n - 1]);
            printf("     n=%2d  ", n); print_i128(got);
            printf("  %s\n", ok ? "ok" : "MISMATCH");
            if (!ok) bad = 1;
...
static void print_checked_result(int label, i128 value)
{
...
int main(int argc, char **argv)
{
    ...
    if (i < argc && strcmp(argv[i], "walk") == 0) {
        int steps, target[3], k;
        i128 value;
        if (i + 5 != argc) {
            usage(argv[0]);
            return EXIT_FAILURE;
        ...
        steps = (int)parse_long_range(
            argv[i + 1], MIN_WALK_STEPS, MAX_WALK_STEPS,
            "walk length must be an integer from 1 to 18");
        for (k = 0; k < 3; k++)
            target[k] = (int)parse_long_range(
                argv[i + 2 + k], -MAX_WALK_STEPS, MAX_WALK_STEPS,
                "target coordinates must be integers from -18 to 18");
        if (!on_lattice(target[0], target[1], target[2]))
            die("target is not an f.c.c. lattice point");
        value = count_walks(steps, target, cutoff, verbose);
        print_checked_result(steps, value);
        return EXIT_SUCCESS;
    }
