...
static void pick_three_primes(uint64_t mods[NMOD]) {
    /* Verification build: use a completely different prime set than the main file.
     * Main file picks primes descending near 2^61.
     * This file picks primes ascending from 2^60+1.
     */
    uint64_t x = (1ULL << 60) + 1ULL;
    int f = 0;
    while (f < NMOD && x < (1ULL << 61) - 1ULL) {
        if ((x & 1ULL) && is_prime_u64(x)) mods[f++] = x;
        x += 2ULL;
    }
    if (f != NMOD) {
        fprintf(stderr, "failed to pick verify primes\n");
        exit(1);
    }
}
...
#ifdef _OPENMP
static void hash_merge_into(Hash *dst, const Hash *src, uint64_t mod) {
    for (size_t i = 0; i < src->cap; i++) {
        if (!src->tab[i].used) continue;
        hash_add(dst, src->tab[i].key, src->tab[i].v, mod);
    }
}
#endif
...
static Problem build_problem_with_primes(int n, const int *pr, int pc) {
    ...
    P.max_oo = (n & 1) ? 1 : 0;
    P.total_even = n / 2;
    P.total_odd = (n - 1) - P.total_even;

    for (int i = 0; i < k; i++) {
        P.type_mask[i] = (uint16_t)pair[i][0];
        P.counts[i] = pair[i][1];
        P.parity[i] = (P.type_mask[i] & 1u) ? 0 : 1; /* bit0 is prime 2 if even */
        P.self_compat[i] = 0;
        if (bigbit >= 0 && P.type_mask[i] == (uint16_t)(1u << bigbit)) {
            P.self_compat[i] = 1;
        }
    }
...
/* Necessary-condition pruning for extending by exactly steps_left positions.
 * parity encoding in this file: 0=even, 1=odd.
 */
static inline int feasible_extension(int rem_odd, int rem_even, int last_parity, int oo_used, int max_oo, int steps_left) {
    if (steps_left < 0) return 0;
    if (rem_odd < 0 || rem_even < 0) return 0;
    if (steps_left == 0) return oo_used <= max_oo;

    int x_min = steps_left - rem_even;
    if (x_min < 0) x_min = 0;
    int x_max = rem_odd < steps_left ? rem_odd : steps_left;

    for (int x = x_min; x <= x_max; x++) {
        int y = steps_left - x;
        if (y < 0 || y > rem_even) continue;
        ...
        int min_extra_oo = (last_parity == 1) ? (x - y) : (x - (y + 1));
        if (min_extra_oo < 0) min_extra_oo = 0;
        if (oo_used + min_extra_oo <= max_oo) return 1;
    }
    return 0;
}

static inline uint64_t pack_key(uint64_t used_code, int last_plus, int oo, int used_even, int k, int oo_states, int even_states) {
    uint64_t x = used_code * (uint64_t)(k + 1) + (uint64_t)last_plus;
    x = x * (uint64_t)oo_states + (uint64_t)oo;
    x = x * (uint64_t)even_states + (uint64_t)used_even;
    return x;
}

static inline void unpack_key(uint64_t key, int k, int oo_states, int even_states, uint64_t *used_code, int *last_plus, int *oo, int *used_even) {
    *used_even = (int)(key % (uint64_t)even_states);
    key /= (uint64_t)even_states;
    *oo = (int)(key % (uint64_t)oo_states);
    key /= (uint64_t)oo_states;
    *last_plus = (int)(key % (uint64_t)(k + 1));
    *used_code = key / (uint64_t)(k + 1);
}

static void expand_state_to_hash(const Problem *P, int depth, int step, int oo_limit, int oo_states, int even_states,
                                 uint64_t used_code, int last_plus, int oo, int used_even,
                                 uint64_t val, Hash *dst, uint64_t mod) {
    int used_cnt[MAXK];
    for (int t = 0; t < P->k; t++) {
    ...
    int used_odd = step - used_even;
    int rem_odd_base = P->total_odd - used_odd;
    int rem_even_base = P->total_even - used_even;
    while (cand) {
        ...
        int noo = oo;
        if (last_plus != 0) {
            int lp = P->parity[last_plus - 1];
            int np = P->parity[t];
            if (lp == 0 && np == 0) continue;
            if (lp == 1 && np == 1) {
                noo++;
                if (noo > oo_limit) continue;
            }
        }

        uint64_t ncode = used_code + P->strides[t];
        int rem_odd = rem_odd_base - (P->parity[t] == 1 ? 1 : 0);
        int rem_even = rem_even_base - (P->parity[t] == 0 ? 1 : 0);
        int steps_left = depth - (step + 1);
        if (!feasible_extension(rem_odd, rem_even, P->parity[t], noo, oo_limit, steps_left)) continue;
        int nused_even = used_even + (P->parity[t] == 0 ? 1 : 0);
        uint64_t nkey = pack_key(ncode, t + 1, noo, nused_even, P->k, oo_states, even_states);
        hash_add(dst, nkey, val, mod);
    }
}

static void build_layer(const Problem *P, int depth, uint64_t mod, int oo_limit, Hash *out) {
    int oo_states = oo_limit + 1;
    int even_states = P->total_even + 1;
    Hash cur, nxt;
    hash_init(&cur, 1 << 15);
    hash_init(&nxt, 1 << 15);
#ifdef _OPENMP
    ...
    uint64_t one = 1;
    uint64_t root = pack_key(0, 0, 0, 0, P->k, oo_states, even_states);
    hash_add(&cur, root, one, mod);
...
#pragma omp for schedule(dynamic, 256)
                for (size_t i = 0; i < cur.cap; i++) {
                    ...
                    uint64_t used_code;
                    int last_plus, oo, used_even;
                    unpack_key(cur.tab[i].key, P->k, oo_states, even_states, &used_code, &last_plus, &oo, &used_even);
                    expand_state_to_hash(P, depth, step, oo_limit, oo_states, even_states,
                                         used_code, last_plus, oo, used_even, cur.tab[i].v, lh, mod);
                }
            }
            for (int t = 0; t < nth; t++) hash_merge_into(&nxt, &locals[t], mod);
        } else {
            for (size_t i = 0; i < cur.cap; i++) {
                if (!cur.tab[i].used) continue;
                uint64_t used_code;
                ...
                unpack_key(cur.tab[i].key, P->k, oo_states, even_states, &used_code, &last_plus, &oo, &used_even);
                expand_state_to_hash(P, depth, step, oo_limit, oo_states, even_states,
                                     used_code, last_plus, oo, used_even, cur.tab[i].v, &nxt, mod);
            }
        }
#else
        for (size_t i = 0; i < cur.cap; i++) {
...
static uint64_t multiply_label_factor_mod(uint64_t r, const Problem *P, uint64_t mod) {
    for (int i = 0; i < P->k; i++) {
        for (int x = 2; x <= P->counts[i]; x++) {
            r = mul_mod_u64(r, (uint64_t)x, mod);
        }
    }
    return r;
}
...
static uint64_t count_mitm_exact(const Problem *P, uint64_t mod, int target_oo) {
    uint64_t out = 0;
    if (target_oo < 0 || target_oo > P->max_oo) return 0;
    if (P->total_len == 0) {
        return (target_oo == 0) ? 1 : 0;
    }
    ...
    if (left_len == 0 || right_len == 0) {
        Hash all;
        build_layer(P, P->total_len, mod, target_oo, &all);
        int oo_states = target_oo + 1;
        int even_states = P->total_even + 1;
        for (size_t i = 0; i < all.cap; i++) {
            if (!all.tab[i].used) continue;
            uint64_t uc;
            int lastp, oo, ue;
            unpack_key(all.tab[i].key, P->k, oo_states, even_states, &uc, &lastp, &oo, &ue);
            (void)uc; (void)lastp; (void)ue;
            if (oo != target_oo) continue;
            out = add_mod(out, all.tab[i].v, mod);
        }
        hash_clear(&all);
        return out;
    }
    ...
    Hash left, right;
    build_layer(P, left_len, mod, target_oo, &left);
    build_layer(P, right_len, mod, target_oo, &right);
    ...
    for (size_t i = 0; i < left.cap; i++) {
        ...
        uint64_t uc;
        int la, ooL, used_even_left;
        unpack_key(left.tab[i].key, P->k, oo_states, even_states, &uc, &la, &ooL, &used_even_left);
        if (la == 0 && P->total_len > 0) continue;

        uint64_t rc = P->total_code - uc;
        int used_even_right = P->total_even - used_even_left;

        for (int ci = 0; ci < compat_len[la - 1]; ci++) {
            int rb = compat_idx[la - 1][ci];
            int cross = (P->parity[la - 1] == 1 && P->parity[rb - 1] == 1) ? 1 : 0;
            for (int ooR = 0; ooR <= target_oo; ooR++) {
                if (ooL + ooR + cross != target_oo) continue;
                uint64_t rkey = pack_key(rc, rb, ooR, used_even_right, P->k, oo_states, even_states);
                uint64_t *rv = hash_get(&right, rkey);
                if (!rv) continue;
                uint64_t add = mul_mod_u64(left.tab[i].v, *rv, mod);
                out = add_mod(out, add, mod);
            }
        }
    }
...
static uint64_t count_mitm(const Problem *P, uint64_t mod) {
    uint64_t r0 = count_mitm_exact(P, mod, 0);
    uint64_t r1 = (P->max_oo > 0) ? count_mitm_exact(P, mod, 1) : 0;
    return add_mod(r0, r1, mod);
}

static void compute_a086595_problem(const Problem *P, const uint64_t mods[NMOD], BigInt *ans) {
    uint64_t r[NMOD];
    for (int i = 0; i < NMOD; i++) {
        uint64_t ri = count_mitm(P, mods[i]);
        r[i] = multiply_label_factor_mod(ri, P, mods[i]);
    }
    crt_reconstruct(r, mods, ans);
}

static void compute_a086595_problem_verbose(const Problem *P, const uint64_t mods[NMOD], BigInt *ans) {
    uint64_t r[NMOD];
    for (int i = 0; i < NMOD; i++) {
        fprintf(stderr, "[mod %d/%d] start\n", i + 1, NMOD);
        fflush(stderr);
        uint64_t ri = count_mitm(P, mods[i]);
        r[i] = multiply_label_factor_mod(ri, P, mods[i]);
        fprintf(stderr, "[mod %d/%d] done\n", i + 1, NMOD);
        fflush(stderr);
    }
    crt_reconstruct(r, mods, ans);
}
...
int main(int argc, char **argv) {
    (void)argc;
    (void)argv;
    const char *out_path = "/Users/s_manyama/Desktop/086595_4/b086595_verify.txt";
    ...
    Problem probs[37];
    for (int n = 1; n <= nmax; n++) probs[n] = build_problem_with_primes(n, primes, pc);
    ...
    FILE *fp = fopen(out_path, start_n == 1 ? "w" : "a");
    if (!fp) {
    ...
    for (int n = start_n; n <= nmax; n++) {
        if (n == 35) {
            ans.len = 0;
            fprintf(stderr, "computing n=35 (verbose mode)\n");
            fflush(stderr);
            compute_a086595_problem_verbose(&probs[n], mods, &ans);
        } else {
            ans.len = 0;
            compute_a086595_problem(&probs[n], mods, &ans);
        }
