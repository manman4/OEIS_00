...
static bool problem_init(Problem *problem, int n) {
    ...
    problem->minimum_image = malloc(minimum_image_bytes);
...
static Result solve(int n) {
    ...
    BranchResult *branches = calloc(1, branch_bytes);
    if (branches == NULL) {
        problem_destroy(&problem);
        fprintf(stderr, "memory allocation failed\n");
        exit(EXIT_FAILURE);
...
#pragma omp parallel for schedule(dynamic, 1)
    for (int first = 0; first <= last_first; ++first) {
        ...
        search.chosen = malloc(n_int_bytes);
        search.image = malloc(n_int_bytes);
        search.next_square = malloc(level_int_bytes);
        search.attacked_count = malloc(level_int_bytes);
        search.attacked_stack = malloc(attacked_stack_bytes);
